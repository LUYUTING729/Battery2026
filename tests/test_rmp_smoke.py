...
class TestRMPSmoke(unittest.TestCase):
    ...
    def test_restricted_master_mip_or_skip(self):
        instance = InstanceData(
            instance_id="x",
            customers={"c1": Customer("c1", 1.0)},
            depots={"d1": Depot("d1", 1)},
        ...
        self.assertGreaterEqual(rmp.add_columns(cols), 1)
        rmp.solve()
        heur = rmp.solve_restricted_mip(time_limit_s=2.0)
        self.assertTrue(heur.feasible)
        self.assertEqual(len(heur.routes), 1)
        self.assertAlmostEqual(heur.obj_val, 3.0)
